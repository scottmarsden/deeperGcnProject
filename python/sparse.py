...
class GSpmm(th.autograd.Function):
    ...
    def forward(ctx, graph, X, norm, num_vcount, dim):
        res = gp_apis.gp_gspmm(graph, X, num_vcount, dim, "sum", 0, norm)  # do not specify the reduce operation
    ...
    def backward(ctx, dZ):
        ...
        res = gp_apis.gp_gspmm(graph, dZ, num_vcount, dim, "sum", 1, norm)  # do not specify the reduce operation
