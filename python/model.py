...
#Main layer code similar to  the gcnconv implementation from class
class DeeperGCN(torch.nn.Module):
    def __init__(self, args):
        ...
        #This is the res+, res, or GCN discussed in the paper 
        #Plain GCN performs better with less layers
        #RES and RES+ perform with more accuracy the more layers there are
	...
	#Aggregation method: add, mean, max, etc
        graph_pooling = args.graph_pooling
	
	
	#Prints the number of layers behing used and which of the aggregation methods are goin to be used
        print('The number of layers {}'.format(self.num_layers),
              'Aggr aggregation method {}'.format(aggr),
              'block: {}'.format(self.block))
              
        #Prints which version we are going to runif its Plain GCN, simple GCN, RES GCN, and RES+ GCN as sepcified in the paper
        if self.block == 'res+':
        ...
        if self.add_virtual_node:
            ...
            for layer in range(self.num_layers - 1):
                self.mlp_virtualnode_list.append(MLP([hidden_channels]*3,
                                                     norm=norm))
        #We run this for each layer we use
        for layer in range(self.num_layers):
            if conv == 'gen':
            #Graph convulution layer
        ...
        #This is where we select the aggregation function that will be used
	...
	#Trained graph after aggregation and normalization
        self.graph_pred_linear = torch.nn.Linear(hidden_channels, num_tasks)

    #Forward function for computing outputs
    ...
    #Function that prints the parameters used
